...
const EventCard = ({ eventId, title, date, location, organizerName, onRegister }) => {
  ...
  return (
    <Link
      to={`/events/${eventId}`}
      className="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-2xl transition-shadow duration-300 ease-in-out flex flex-col group"
    >
      <div className="relative">
        <img
          src={`https://placehold.co/600x400/a0c4ff/333333?text=${title.replace(/\s+/g, '+')}`}
          alt={title}
